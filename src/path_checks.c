...
static int wrongchar_check(char *path)
{
	...
	while (i < (size - 4))
	{
		if (ft_isalnum(path) && path[i] != '_' && path[i] != '-')
			return (1);
		i++;
	}
	return 0;
